...
type RequestType = 'analysis' | 'connection-test' | 'interpret';
...
function pickRequestType(raw: unknown): RequestType {
  if (raw === 'connection-test') return 'connection-test';
  if (raw === 'interpret') return 'interpret';
  return 'analysis';
}
...
async function callOpenAICompatible(
  ...
  if (!upstreamResponse.ok) {
    const upstreamMessage =
      upstreamJson?.error?.message || upstreamText || `Upstream request failed with status ${upstreamResponse.status}`;
...
async function callOllamaChat(
  ...
  if (!upstreamResponse.ok) {
    const upstreamMessage = upstreamJson?.error || upstreamText || `Upstream request failed with status ${upstreamResponse.status}`;
    throw new Error(`AI provider error (${upstreamResponse.status}): ${upstreamMessage}`);
  }
...
function buildPrompts(
  ...
  if (requestType === 'interpret') {
    return {
      systemPrompt:
        language === 'zh'
          ? '你是一名资深安全架构与合规审核专家。任务是把单个控制项/条款解释为可执行要求，输出必须具体、可审计、可落地，不得编造输入中不存在的义务。'
          : 'You are a senior security architecture and compliance audit expert. Explain a single control/clause into execution-ready guidance. Output must be specific, auditable, and grounded in provided input without inventing obligations.',
      instruction:
        language === 'zh'
          ? [
              '请对当前控制项生成结构化解读，使用 Markdown，必须包含这些小节：',
              '1) 控制项核心意图（2-4条）',
              '2) 适用边界（何时适用/不适用）',
              '3) 实施要点（流程/技术/组织）',
              '4) 审计证据清单（至少6条，可核查）',
              '5) 常见误区与反模式',
              '6) 基于当前评估状态的下一步（7天/30天/90天）',
              '7) 可直接写入系统的评估备注模板（80-150字）',
              '',
              '要求：',
              '- 必须引用输入中的控制项文本和上下文。',
              '- 法规类条款要明确义务强度与适用对象。',
          ...
          : [
              'Generate a structured interpretation in Markdown with these sections:',
              '1) Core intent (2-4 bullets)',
              '2) Applicability boundaries (when applicable / not applicable)',
              '3) Implementation essentials (process / technology / organization)',
              '4) Audit evidence checklist (at least 6 verifiable items)',
              '5) Common pitfalls and anti-patterns',
              '6) Next steps by current status (7/30/90 days)',
              '7) A ready-to-paste assessment note template (80-150 words)',
              '',
              'Requirements:',
              '- Ground every point in provided control text/context.',
              '- For regulatory clauses, explicitly state obligation strength and subject.',
              '- Keep it execution-oriented, avoid generic advice.',
              '',
              'Input JSON:',
              JSON.stringify(payload, null, 2),
            ].join('\n'),
  ...
  return {
    systemPrompt:
      language === 'zh'
        ...
        : 'You are a senior cybersecurity compliance and governance expert. Write from a CISO/Security Director perspective, focusing on business risk, governance priorities, budget trade-offs, and execution-ready actions grounded in provided assessment data.',
    instruction:
      language === 'zh'
        ? [
...
export async function POST(request: NextRequest) {
  try {
    ...
    const payload: PromptPayload = {
      ...
      assessmentStatusDistribution: body?.assessmentStatusDistribution || {},
      prioritizedGaps: Array.isArray(body?.prioritizedGaps) ? body.prioritizedGaps : [],
      businessContext: body?.businessContext || {},
      controlContext: body?.controlContext || {},
    };
