...
功能热点：
- 左侧始终保留框架导航，快速切换分类与子分类。
- 右侧聚焦末级控制项，直接完成“评估状态 + 评估备注”。
- 不再依赖单独答题页，框架页即评估入口。

### 2) 控制项 AI 解读（右侧抽屉）

![控制项AI解读](docs/screenshots/ai-interpret.png)

功能热点：
- 每个控制项右侧一个 AI 解读按钮 + 右侧抽屉面板。
- 不打断操作流。用户不用跳转，始终在当前控制项上下文中。
- 减少认知负担。看完解读可直接改“评估状态”和“评估备注”。
- 内置针对性优化的 prompt 模板输出结构化内容，帮助快速理解控制项。
- 支持一键写入评估备注，减少重复录入。

### 3) 全局 AI 配置弹窗（一次配置，全局复用）

![AI配置弹窗](docs/screenshots/ai-config-modal.png)

功能热点：
- AI 报告页与 AI 解读共享同一配置入口。
- 一次配置、全局复用，避免每个页面重复填 key。
- 支持 `OpenAI / Anthropic / Ollama / DeepSeek / MiniMax / Kimi / 通用适配`。
- 通用适配支持多协议（含 `anthropic-messages`），可接官方与中转平台。
...
功能热点：
- 汇总覆盖率、状态分布、成熟度指数，便于管理层快速查看现状。
- 支持 `JSON/CSV` 导出，便于审计归档与二次分析。
- 一键进入 AI 安全专家，直接基于已评估结果生成改进方案。
...
功能热点：
- 结合企业行业、规模、预算、KPI 与目标周期生成分阶段行动计划。
- 输出面向 CISO 的结构化建议（差距、优先级、90天路线、预算分配）。
- 支持导出 HTML 报告，可直接用于汇报与复盘。
